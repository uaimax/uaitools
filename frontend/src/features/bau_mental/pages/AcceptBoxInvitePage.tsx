...
export default function AcceptBoxInvitePage() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      if (!token) {
        ...
        setTimeout(() => navigate("/bau-mental"), 3000);
        return;
      }

      try {
        // Verificar token sem autenticação
        const response = await apiClient.get<InviteInfo>("/bau-mental/invites/verify/", {
          params: { token },
        });
        ...
        if (user) {
          ...
          if (user.email.toLowerCase() !== response.data.email.toLowerCase()) {
            ...
            setTimeout(() => navigate("/bau-mental"), 5000);
            return;
          }
  ...
  const acceptInvite = async () => {
    ...
    setStatus("loading");
    try {
      const response = await apiClient.post("/bau-mental/invites/accept/", { token });
      setStatus("success");
          ...
          <CardContent className="space-y-4">
            ...
            {status === "success" && (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-xs text-muted-foreground">
                  Redirecionando para Baú Mental...
                </p>
              </div>
            )}
            ...
            {status === "error" && (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <Button onClick={() => navigate("/bau-mental")} variant="outline">
                  Ir para Baú Mental
                </Button>
              </div>
            )}
