...
export function useBoxShares(boxId: string | null) {
  return useQuery<BoxShare[]>({
    queryKey: ["supbrainnote", "boxes", boxId, "shares"],
    queryFn: async () => {
      if (!boxId) return [];
      const response = await apiClient.get(`/supbrainnote/boxes/${boxId}/shares/`);
...
export function useShareBox() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await apiClient.post(`/supbrainnote/boxes/${boxId}/share/`, {
        user_id,
        email,
        permission,
      });
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["supbrainnote", "boxes", variables.boxId, "shares"],
      });
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await apiClient.patch(
        `/supbrainnote/boxes/${boxId}/shares/${shareId}/`,
        { permission }
      );
      return response.data;
    },
...
export function useRemoveShare() {
  ...
  return useMutation({
    mutationFn: async ({ boxId, shareId }: { boxId: string; shareId: string }) => {
      await apiClient.delete(`/supbrainnote/boxes/${boxId}/shares/${shareId}/`);
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["supbrainnote", "boxes", variables.boxId, "shares"],
