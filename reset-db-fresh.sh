#!/bin/bash
# Script para resetar banco e aplicar migrations do zero (sem preservar dados)

set -e

cd "$(dirname "$0")/backend"

echo "🔄 Reset Completo do Banco - Criar do Zero"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

if [ ! -d "venv" ]; then
    echo -e "${RED}❌ Venv não encontrado${NC}"
    exit 1
fi

source venv/bin/activate

echo -e "${YELLOW}⚠️  ATENÇÃO: Este script vai:${NC}"
echo "   1. Dropar TODAS as tabelas do banco"
echo "   2. Limpar histórico de migrations"
echo "   3. Aplicar todas as migrations do zero"
echo "   4. Perder TODOS os dados existentes"
echo ""
read -p "Deseja continuar? (s/N): " -n 1 -r
echo ""

if [[ ! $REPLY =~ ^[Ss]$ ]]; then
    echo -e "${YELLOW}Operação cancelada.${NC}"
    exit 0
fi

echo ""
echo -e "${BLUE}1️⃣  Dropar todas as tabelas...${NC}"

# Usar flush do Django para limpar dados, depois dropar tabelas
python manage.py flush --no-input 2>/dev/null || true

# Dropar todas as tabelas usando SQL
python manage.py dbshell <<'EOF' 2>/dev/null || true
-- Dropar todas as tabelas em cascata
DO $$ 
DECLARE 
    r RECORD;
BEGIN
    -- Desabilitar triggers temporariamente
    SET session_replication_role = 'replica';
    
    -- Dropar todas as tabelas
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') 
    LOOP
        EXECUTE 'DROP TABLE IF EXISTS ' || quote_ident(r.tablename) || ' CASCADE';
    END LOOP;
    
    -- Reabilitar triggers
    SET session_replication_role = 'origin';
END $$;
EOF

echo -e "${GREEN}   ✅ Tabelas removidas${NC}"

# Limpar histórico de migrations
echo ""
echo -e "${BLUE}2️⃣  Limpando histórico de migrations...${NC}"
python manage.py dbshell <<'EOF' 2>/dev/null || true
-- Dropar tabela de migrations se existir
DROP TABLE IF EXISTS django_migrations CASCADE;
EOF

echo -e "${GREEN}   ✅ Histórico limpo${NC}"

# Aplicar todas as migrations do zero
echo ""
echo -e "${BLUE}3️⃣  Aplicando todas as migrations do zero...${NC}"
echo -e "${YELLOW}   Isso pode levar alguns segundos...${NC}"
echo ""

# Usar --run-syncdb para criar tabelas básicas primeiro
python manage.py migrate --run-syncdb --verbosity=1

if [ $? -eq 0 ]; then
    echo ""
    echo -e "${GREEN}✅ Migrations aplicadas com sucesso!${NC}"
    echo ""
    echo -e "${BLUE}4️⃣  Verificando estado final...${NC}"
    echo ""
    echo "Migrations do bau_mental:"
    python manage.py showmigrations bau_mental 2>&1 | tail -10
    echo ""
    
    # Verificar tabelas criadas
    echo -e "${BLUE}5️⃣  Verificando tabelas criadas...${NC}"
    TABLES=$(python manage.py dbshell <<'EOF' 2>/dev/null | grep "bau_mental" || echo ""
\dt bau_mental*
EOF
)
    
    if [ -n "$TABLES" ]; then
        echo "$TABLES" | sed 's/^/   /'
        echo -e "${GREEN}   ✅ Tabelas bau_mental criadas corretamente${NC}"
    else
        echo -e "${YELLOW}   ⚠️  Não foi possível verificar tabelas (pode ser normal)${NC}"
    fi
    
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo -e "${GREEN}✅ Processo concluído!${NC}"
    echo ""
    echo "Próximos passos:"
    echo "  1. Teste os endpoints: curl http://localhost:8000/api/v1/bau-mental/boxes/"
    echo "  2. Teste o frontend: http://localhost:5173/bau-mental"
    echo "  3. Execute os testes: python manage.py test apps.bau_mental"
else
    echo ""
    echo -e "${RED}❌ Erro ao aplicar migrations${NC}"
    echo ""
    echo "Tente executar manualmente:"
    echo "  python manage.py migrate"
    exit 1
fi
