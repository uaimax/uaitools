...
# Limpar histórico de migrations completamente
echo ""
echo -e "${BLUE}2️⃣  Limpando histórico de migrations...${NC}"
python manage.py dbshell <<'EOF' 2>/dev/null || true
-- Se a tabela existe, limpar
...
python manage.py migrate --verbosity=1 --run-syncdb
...
if [ $? -eq 0 ]; then
    ...
    python manage.py dbshell <<'EOF' 2>/dev/null | grep -E "bau_mental" || echo "   (Nenhuma tabela bau_mental encontrada ou erro ao verificar)"
\dt bau_mental*
EOF
