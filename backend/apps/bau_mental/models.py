...
def audio_upload_path(instance: "Note", filename: str) -> str:
    ...
    # Formato: bau_mental/audios/{workspace_id}/{year}/{month}/{day}/{filename}
    from django.utils import timezone
    import uuid
...
class Box(UUIDPrimaryKeyMixin, WorkspaceModel):
    ...
    keywords = models.CharField(
        ...
        help_text=_("Palavras-chave separadas por vírgula (ex: evento, festa, réveillon)"),
    )
    note_count = models.IntegerField(
        default=0,
        verbose_name=_("Contagem de notas"),
        help_text=_("Contagem em cache (atualizada via trigger)"),
    )
    last_note_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_("Última nota em"),
        help_text=_("Data da última nota adicionada (atualizada via trigger)"),
    )
    summary = models.TextField(
        null=True,
        blank=True,
        verbose_name=_("Resumo"),
        help_text=_("Resumo gerado por IA (cacheado)"),
    )
    summary_generated_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_("Resumo gerado em"),
    )
    summary_stale = models.BooleanField(
        default=False,
        verbose_name=_("Resumo desatualizado"),
        help_text=_("True se resumo precisa ser regenerado"),
    )
...
class Note(UUIDPrimaryKeyMixin, WorkspaceModel):
    ...
    from apps.bau_mental.storage import BauMentalAudioStorage
    ...
    # search_vector é uma coluna gerada automaticamente no PostgreSQL
    # Não precisa ser definida como campo Django, é criada via migration SQL
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        # Marcar resumo como desatualizado quando nota é criada/editada/deletada
        old_box_id = None
        old_deleted_at = None
        if self.pk:
            try:
                old_note = Note.objects.get(pk=self.pk)
                old_box_id = old_note.box_id if old_note.box else None
                old_deleted_at = old_note.deleted_at
            except Note.DoesNotExist:
                pass
        
        super().save(*args, **kwargs)
        
        # Marcar resumo como desatualizado se box mudou, foi criada, ou foi deletada/restaurada
        # Usar import local para evitar circular
        from apps.bau_mental.models import Box
        if self.box_id:
            Box.objects.filter(id=self.box_id).update(summary_stale=True)
        if old_box_id and old_box_id != self.box_id:
            Box.objects.filter(id=old_box_id).update(summary_stale=True)
        # Se deleted_at mudou (soft delete ou restore), invalidar cache
        if old_deleted_at != self.deleted_at and self.box_id:
            Box.objects.filter(id=self.box_id).update(summary_stale=True)
...
class Thread(UUIDPrimaryKeyMixin, WorkspaceModel):
    """Thread de conversa contextual com o acervo (estilo ChatGPT)."""

    title = models.CharField(
        max_length=255,
        verbose_name=_("Título"),
        help_text=_("Título da thread (gerado automaticamente ou manual)"),
    )
    box = models.ForeignKey(
        "bau_mental.Box",
        on_delete=models.SET_NULL,
        ...
        verbose_name=_("Caixinha"),
        help_text=_("Thread contextual de uma caixinha específica"),
    )
    boxes = models.ManyToManyField(
        "bau_mental.Box",
        blank=True,
        related_name="multi_threads",
        verbose_name=_("Caixinhas"),
        help_text=_("Thread contextual de múltiplas caixinhas"),
    )
    is_global = models.BooleanField(
        default=False,
        verbose_name=_("Global"),
        help_text=_("Thread global (todas as notas)"),
    )
    pinned_summary = models.TextField(
        null=True,
        blank=True,
        verbose_name=_("Síntese fixada"),
        help_text=_("Resposta fixada como síntese"),
    )
    created_by = models.ForeignKey(
        "accounts.User",
        on_delete=models.SET_NULL,
    ...
    last_message_at = models.DateTimeField(
        ...
        help_text=_("Atualizado automaticamente quando nova mensagem é adicionada"),
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["workspace", "box", "last_message_at"]),
            models.Index(fields=["workspace", "is_global", "last_message_at"]),
            models.Index(fields=["created_by"]),
        ]

    def __str__(self) -> str:
        """Representação string da thread."""
        return f"{self.title} ({self.workspace.name})"


class ThreadMessage(UUIDPrimaryKeyMixin, WorkspaceModel):
    """Mensagem em uma thread (usuário ou IA)."""
    ...
    notes_referenced = models.ManyToManyField(
        Note,
        blank=True,
        related_name="referenced_in_messages",
        verbose_name=_("Notas referenciadas"),
        help_text=_("Notas citadas nesta mensagem"),
    )
    created_by = models.ForeignKey(
        "accounts.User",
        on_delete=models.SET_NULL,
    ...
    class Meta:
        verbose_name = _("Mensagem de Thread")
        verbose_name_plural = _("Mensagens de Threads")
        ordering = ["created_at"]
        indexes = [
            models.Index(fields=["thread", "created_at"]),
            models.Index(fields=["workspace", "thread"]),
        ]

    def __str__(self) -> str:
        """Representação string da mensagem."""
        return f"{self.get_role_display()} - {self.thread.title} ({self.created_at.strftime('%d/%m/%Y %H:%M')})"
