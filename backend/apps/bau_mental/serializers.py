"""Serializers for bau_mental app."""
...
class BoxSerializer(WorkspaceSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["id", "workspace_id", "created_at", "updated_at", "note_count", "last_note_at", "summary", "summary_generated_at", "summary_stale"]
...
class ThreadMessageSerializer(WorkspaceSerializer):
    """Serializer para mensagem de thread."""

    notes_referenced = serializers.PrimaryKeyRelatedField(
    ...
    class Meta:
        ...
        read_only_fields = ["id", "workspace_id", "created_at", "updated_at", "created_by"]


class ThreadSerializer(WorkspaceSerializer):
    """Serializer para thread."""

    messages = ThreadMessageSerializer(many=True, read_only=True)
    boxes = serializers.PrimaryKeyRelatedField(
        many=True, queryset=Box.objects.all(), required=False
    )
    created_by_email = serializers.EmailField(source="created_by.email", read_only=True)
    ...
    def get_last_message_preview(self, obj: Thread) -> str | None:
    ...
    def get_messages_count(self, obj: Thread) -> int:
        """Retorna quantidade de mensagens."""
        return obj.messages.count()


class ThreadListSerializer(serializers.ModelSerializer):
    """Serializer simplificado para listagem de threads."""
    ...
    def get_box_name(self, obj: Thread) -> str | None:
        """Retorna nome da caixinha ou None."""
        return obj.box.name if obj.box else None


class ThreadCreateSerializer(serializers.Serializer):
    """Serializer para criar thread."""

    title = serializers.CharField(max_length=255, required=False)
    box_id = serializers.UUIDField(required=False)
    ...
    def validate(self, attrs: dict) -> dict:
        """Valida que apenas uma opção de contexto é fornecida."""
        box_id = attrs.get("box_id")
        box_ids = attrs.get("box_ids", [])
        is_global = attrs.get("is_global", False)

        contexts = sum([bool(box_id), bool(box_ids), is_global])
        if contexts > 1:
            raise serializers.ValidationError(
                "Apenas uma opção de contexto pode ser fornecida: box_id, box_ids ou is_global."
            )
        return attrs

    def create(self, validated_data: dict) -> "Thread":
        """Cria uma nova thread."""
        from apps.bau_mental.models import Thread, Box
        from apps.bau_mental.utils import get_or_create_workspace_for_user

        request = self.context.get("request")
        if not request:
            raise serializers.ValidationError("Request é obrigatório no contexto")

        workspace = get_or_create_workspace_for_user(request)
        if not workspace:
            raise serializers.ValidationError("Workspace é obrigatório")

        title = validated_data.get("title", "Nova conversa")
        box_id = validated_data.get("box_id")
        box_ids = validated_data.get("box_ids", [])
        ...
        # Associar caixinha única se fornecido
        if box_id:
            try:
                box = Box.objects.get(id=box_id, workspace=workspace)
                thread.box = box
                thread.save(update_fields=["box"])
            except Box.DoesNotExist:
                pass  # Se não encontrar, deixa sem caixinha

        # Associar múltiplas caixinhas se fornecido
        if box_ids:
            boxes = Box.objects.filter(id__in=box_ids, workspace=workspace)
            thread.boxes.set(boxes)

        return thread


class ThreadMessageCreateSerializer(serializers.Serializer):
    """Serializer para criar mensagem em thread."""
