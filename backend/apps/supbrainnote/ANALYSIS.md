# SupBrainNote App — Análise do Módulo
...
O app `supbrainnote` é um módulo completo para organização de anotações por áudio com classificação automática em "caixinhas" (categorias) e consulta inteligente via IA.
...
- `POST /api/v1/supbrainnote/notes/upload/` - Upload de áudio (rate limit: 10/hora)
- `POST /api/v1/supbrainnote/notes/record/` - Gravação direta (rate limit: 10/hora)
...
**Endpoints:**
- `POST /api/v1/supbrainnote/query/ask/` - Consulta com IA (rate limit: 50/hora)
...
- **Uploads:** 10 uploads/hora por workspace (`SupBrainNoteUploadThrottle`)
- **Consultas:** 50 consultas/hora por workspace (`SupBrainNoteQueryThrottle`)
