# Arquitetura do Módulo bau_mental
...
class Note(WorkspaceModel):
    ...
    audio_file = models.FileField(
        upload_to="bau_mental/audios/%Y/%m/%d/",
...
- `POST /api/v1/bau-mental/notes/` - Cria anotação
...
- `POST /api/v1/bau-mental/notes/upload/` - Upload de áudio
- `POST /api/v1/bau-mental/notes/record/` - Gravação direta
...
**Endpoints:**
- `POST /api/v1/bau-mental/query/ask/` - Consulta com IA
...
class NoteViewSet(WorkspaceViewSet):
    ...
    throttle_scope = "bau_mental_upload"  # Limite específico
```
...
```
1. Usuário faz upload → POST /api/v1/bau-mental/notes/upload/
...
```
1. Usuário grava → POST /api/v1/bau-mental/notes/record/
...
AWS_STORAGE_BUCKET_NAME=bau_mental-audios
